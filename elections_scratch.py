...
class Election:
    ...
    def results_for(self, riding: str, party: str) -> Optional[int]:
        ...
        1
        """
        if riding not in self._results or party not in self._results[party]: 
            pass
        else: 
    ...
    def riding_winners(self, riding: str) -> List[str]:
        ...
        ['pc']
        """
        r = [list(self._results[riding].keys())[0]]
        for par in self._results[riding]: 
    ...
    def popular_vote(self) -> Dict[str, int]:
        ...
        for rid in self._results: 
            for par in self._results[rid]: 
                if par not in result: 
                    result[par] = self._results[rid][par]
                else: 
                    result[par] += self._results[rid][par]
        return r
    ...
    def party_seats(self) -> Dict[str, int]:
        ...
        r = {}
        for rid in self._results: 
            if len(self.riding_winners) == 1: 
                if rid in r: 
                    r[self.riding_winners[0]] += 1
                else: 
                    r[self.riding_winners[0]] = 1
            pass
    ...
    def election_winners(self) -> List[str]:
        ...
        ['pc']
        """
        #revise for empty votes
        if not self.party_seats: 
            return []
        parties = list(self.party_seats.keys())
        r = [parties[0]]
        for par in range(len(parties) + 1): 
            if self.party_seats[parties[par]] > r[0]: 
                r = [self.parties[par]]
            elif self.party_seats[parties[par]] == r[0]: 
                r = r + [self.parties[par]]
            pass
        return r
