...
class User:
    ...
    '''
    def retweet(self, t: Tweet): 
        self.tweet(t.contents)
    
    '''

    def follow(self, other: User) -> None:
        """Record that this User follows <other>.s
    ...
    def verbosity(self, y: int) -> int:
        ...
        a = 0
        for c in self.tweets:
            if c.created_at.year == y:
                a = a + len(c.content)
        return a
    ...
    def retweet(tweet: Tweet, new_date: date) -> Tweet:
        ...
        >>> u1 = User('Diane', 'Welcome. ')
        >>> t2 = u1.retweet(t1, date(2017, 8, 20))
        >>> t2.content
        'David is so cool!'
        >>> t2.userid
        'Diane'
        ...
        """
        return Tweet(User.userid, new_date, tweet.content)
