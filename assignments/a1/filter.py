...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        condition = filter_string[1:]
        # check if filter_string is a number after 'G'/'L'.
        try:
            int(condition)
        except ValueError:
            return data
        ...
        if filter_string[0] == 'L':
            for calls in data:
                if calls.duration < int(condition):
                    valid_calls += [calls]
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        specified in the handout.
        """
        coor_lst = filter_string.split(', ')
        # Map Lower-left corner: -79.697878, 43.576959")
        # Map Upper-right corner: -79.196382, 43.799568")

        # ErrCase: invalid coordinates
        if len(coor_lst) != 4:
            return data

        # ErrCase: list elements not float
        for c in coor_lst:
            try:
                float(c)
            except ValueError:
        ...
        # ErrCase: Coordinates not in map range
        if not ((-79.697878 <= float(coor_lst[2]) <= -79.196382)
                and (-79.697878 <= float(coor_lst[0]) <= -79.196382)
                and (43.576959 <= float(coor_lst[3]) <= 43.799568)
                and (43.576959 <= float(coor_lst[1]) <= 43.799568)
                and (float(coor_lst[0]) <= float(coor_lst[2]))
                and (float(coor_lst[1]) <= float(coor_lst[3]))):
            return data

        r = []
        for calls in data:
            if ((float(coor_lst[0]) <= calls.src_loc[0] <=
                 float(coor_lst[2])
                 and float(coor_lst[1]) <= calls.src_loc[1] <=
                 float(coor_lst[3])) or
                    (float(coor_lst[0]) <= calls.dst_loc[0] <=
                     float(coor_lst[2])
                     and float(coor_lst[1]) <= calls.dst_loc[1] <=
                     float(coor_lst[3]))):
                r += [calls]
        return r
