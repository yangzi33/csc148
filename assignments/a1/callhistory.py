...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        """
        month_year = (call.time.month, call.time.year)
        if month_year not in self.outgoing_calls:
            self.outgoing_calls[month_year] = [call]
        else:
            self.outgoing_calls[month_year] += [call]
    ...
    def register_incoming_call(self, call: Call) -> None:
        ...
        """
        month_year = (call.time.month, call.time.year)
        if month_year not in self.incoming_calls:
            self.incoming_calls[month_year] = [call]
        else:
            self.incoming_calls[month_year] += [call]
